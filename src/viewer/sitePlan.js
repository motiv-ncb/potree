...
import {DXFSitePlanExporter} from "../exporter/DXFSitePlanExporter.js";
...
class SitePlanFakeOctree extends PointCloudTree{
...
export class SitePlanWindow extends EventDispatcher {
    constructor (viewer) {
        super();
    ...
    initListeners () {
        ...
        this.renderArea.mousemove(e => {
            ...
            if (this.mouseIsDown) {
                ...
                if (closest) {
                    ...
                    let info = this.elRoot.find('#sitePlanSelectionProperties');
                    let html = '<table>';
        ...
        $('#closeSitePlanContainer').click(() => {
            this.hide();
        });

        let getSitePlanPoints = (truePosition) => {
        ...
        $('#potree_download_dxf2D_icon').click(() => {
            ...
            const string = DXFSitePlanExporter.toString(points, true);
        ...
        $('#potree_download_dxf3D_icon').click(() => {
            ...
            const string = DXFSitePlanExporter.toString(points);
        ...
        $('#potree_download_las_icon').click(() => {
            ...
            $('#potree_download_sitePlan_link').attr('href', URL.createObjectURL(blob));
        });
    }
    ...
    addPoints (pointcloud, points) {
        ...
        if(!entry){
            entry = new SitePlanFakeOctree(pointcloud);
            this.pointclouds.set(pointcloud, entry);
            this.sitePlanScene.add(entry);
        ...
        $(`#sitePlan_num_points`).html(Utils.addCommas(numPoints));
    ...
    reset () {
        ...
        this.elRoot.find('#sitePlanSelectionProperties').hide();
    ...
    render () {
        ...
        let {renderer, pRenderer, camera, sitePlanScene, scene} = this;
        ...
        pRenderer.render(sitePlanScene, camera, null);
...
export class SitePlanWindowController {
    constructor (viewer) {
        this.viewer = viewer;
        this.sitePlanWindow = viewer.sitePlanWindow;
        this.sitePlan = null;
        this.numPoints = 0;
    ...
    setSitePlan (sitePlan) {
        if (this.sitePlan !== null && this.sitePlan !== sitePlan) {
            this.sitePlan.removeEventListener('marker_moved', this._recompute);
            this.sitePlan.removeEventListener('marker_added', this._recompute);
            this.sitePlan.removeEventListener('marker_removed', this._recompute);
            this.sitePlan.removeEventListener('width_changed', this._recompute);
        }

        this.sitePlan = sitePlan;

        {
            this.sitePlan.addEventListener('marker_moved', this._recompute);
            this.sitePlan.addEventListener('marker_added', this._recompute);
            this.sitePlan.addEventListener('marker_removed', this._recompute);
            this.sitePlan.addEventListener('width_changed', this._recompute);
        }
    ...
    recompute () {
        ...
        for (let pointcloud of this.viewer.scene.pointclouds.filter(p => p.visible)) {
            let request = pointcloud.getPointsInSitePlan(this.sitePlan, null, {
                'onProgress': (event) => {
                    if (!this.enabled) {
                        return;
                    }
